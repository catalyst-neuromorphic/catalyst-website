...
// Full grid: 40x22 = 880 tiles (matches homepage)
const COLS = 40;
const ROWS = 22;
...
// === FOG SCENE (simplified: 3-octave FBM, 4 nodes, no synapses) ===
...
const fogMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    // 3-octave FBM (lighter than homepage's 6)
    float fbm(vec3 p){
    ...
    void main(){
      ...
      // 4 neural nodes (vs homepage's 8)
      float nodes=0.;
      for(int i=0;i<4;i++){
        float fi=float(i);
...
// === TILES (simplified: no per-tile FBM, single fog sample) ===
const GLOW_PAD = 0.06;
...
} // end mobile/reduced-motion guard
